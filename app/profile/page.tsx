...
function LevelGamificationCard({ level, totalPengajuan }: { level: number; totalPengajuan: number }) {
  ...
  return (
    <div className="relative group cursor-pointer overflow-visible z-30 w-full">
      <div className="w-full rounded-2xl border border-steel/20 bg-paper p-5 transition duration-200 hover:border-bridge-gold/60">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="w-full md:w-72 shrink-0">
            ...
            <div className="h-3 w-full rounded-full bg-steel/10 overflow-hidden border border-steel/20">
              <div
                className="h-full bg-bridge-gold rounded-full transition-all duration-500"
...
function SuccessModal({ onClose }: { onClose: () => void }) {
  const AUTO_DISMISS_MS = 1500; // Disesuaikan sedikit agar animasi centang selesai dengan pas
      ...
      <div className="relative w-full max-w-sm overflow-hidden rounded-2xl bg-paper p-6 text-center shadow-xl border border-steel/20 animate-in zoom-in-95 duration-200">
        ...
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-bridge-gold/15 border border-bridge-gold/30">
          <svg viewBox="0 0 52 52" className="h-10 w-10">
            {/* Lingkaran luar yang menggambar/memutar */}
            <circle
              cx="26"
              cy="26"
              r="23"
            ...
            />
            {/* Tanda centang yang menyusul tergambar */}
            <path
              fill="none"
              stroke="currentColor"
              strokeWidth="3.5"
        ...
        {/* Style Animasi SVG stroke & Countdown */}
        <style>{`
          .animate-draw-circle {
          ...
          <div className="flex w-full items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={handleSave}
                className="rounded-xl bg-ink px-5 py-2 text-xs font-semibold text-paper shadow-xs hover:bg-ink/90 transition active:scale-95 flex items-center gap-1.5"
              >
      ...
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-2xl bg-paper shadow-2xl border border-steel/20 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-steel/10 px-6 py-4 shrink-0">
          ...
          <button
            ...
            className="rounded-xl bg-ink px-5 py-2.5 text-xs font-bold text-paper hover:bg-ink/90 transition active:scale-95 flex items-center gap-2"
          >
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-steel/20 bg-paper p-4.5 hover:border-bridge-gold/40 transition">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-steel">Total Kontribusi</span>
            <div className="p-2 rounded-xl bg-bridge-gold/10 text-bridge-gold border border-bridge-gold/30">
              <IconActivity className="w-4 h-4 text-bridge-gold" />
            </div>
          </div>
          <p className="text-2xl font-black text-ink mt-2">38 <span className="text-xs font-semibold text-bridge-gold">+12%</span></p>
          <p className="text-[11px] text-steel/70 mt-1">Aksi publik dalam 30 hari</p>
        </div>

        <div className="rounded-2xl border border-steel/20 bg-paper p-4.5 hover:border-bridge-gold/40 transition">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-steel">Streak Keaktifan</span>
            <div className="p-2 rounded-xl bg-bridge-gold/10 text-bridge-gold border border-bridge-gold/30">
              <IconFlame className="w-4 h-4 text-bridge-gold" />
            </div>
          </div>
          <p className="text-2xl font-black text-ink mt-2">14 Hari</p>
          <p className="text-[11px] text-steel/70 mt-1">Aktif berturut-turut</p>
        </div>

        <div className="rounded-2xl border border-steel/20 bg-paper p-4.5 hover:border-bridge-gold/40 transition">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-steel">Reputasi Publik</span>
            <div className="p-2 rounded-xl bg-bridge-gold/10 text-bridge-gold border border-bridge-gold/30">
              <IconTrophy className="w-4 h-4 text-bridge-gold" />
            </div>
          </div>
          <p className="text-2xl font-black text-ink mt-2">480 <span className="text-xs font-normal text-steel/70">Pts</span></p>
          <p className="text-[11px] text-steel/70 mt-1">Top 5% Mahasiswa Aktif</p>
        </div>

        <div className="rounded-2xl border border-steel/20 bg-paper p-4.5 hover:border-bridge-gold/40 transition">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-steel">Respon Rate</span>
            <div className="p-2 rounded-xl bg-bridge-gold/10 text-bridge-gold border border-bridge-gold/30">
              <IconCheckSquare className="w-4 h-4 text-bridge-gold" />
            </div>
          </div>
          <p className="text-2xl font-black text-bridge-gold mt-2">98%</p>
          <p className="text-[11px] text-steel/70 mt-1">Respon komunikasi cepat</p>
        </div>
      </div>
...
export default function ProfilePage() {
  ...
  const animatedSkillsCount = useAnimatedNumber(skillsList.length);
  const animatedMinatCount = useAnimatedNumber(minatList.length);
  ...
  return (
    <main className="min-h-screen bg-paper text-ink pb-20 overflow-x-visible">
      ...
      {/* Hero Banner Area */}
      <div className="-mt-20 w-full border-b border-steel/20 bg-paper">
        <div className="w-full bg-gradient-to-r from-ink via-ink/90 to-steel relative pt-24 pb-20 sm:pb-24">
        ...
        <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-8 lg:px-12 relative overflow-visible">
          <div className="flex flex-col md:flex-row items-center md:items-end justify-between relative -mt-16 sm:-mt-20 gap-6 pb-4">
            ...
            {/* Avatar Profile (Icon Kamera Selalu Clickable) */}
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5 text-center sm:text-left z-20">
              <div className="relative group shrink-0">
                <div className="h-36 w-36 sm:h-44 sm:w-44 overflow-hidden rounded-full border-4 border-paper bg-steel/10 shadow-md transition-transform duration-200 group-hover:scale-105">
                  {user.foto ? (
                    // eslint-disable-next-line @next/next/no-img-element
                ...
                {/* Tombol Kamera selalu ada & active */}
                <button
                  type="button"
              ...
              <div className="mb-1">
                <h2 className="text-2xl sm:text-3xl font-black text-ink tracking-tight">{user.nama || "Nama Kamu"}</h2>
            ...
            {/* Tombol Edit Profil (Membuka Modal) — hanya tampil di Tab Detail Profil */}
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 rounded-xl px-5 py-3 sm:px-6 text-xs sm:text-sm font-bold transition-all duration-150 active:scale-95 border ${
                      isActive
                        ? "bg-ink border-ink text-paper shadow-md"
                        : "bg-paper border-steel/20 text-steel hover:bg-paper hover:text-ink hover:border-steel/40"
                    }`}
                  >
      ...
      <div className="mx-auto w-full max-w-[1400px] px-4 pt-6 sm:px-8 lg:px-12 overflow-visible">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="rounded-2xl border border-steel/20 bg-paper p-5">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {activeTab === "profile" && (
              <div className="rounded-2xl border border-steel/20 bg-paper p-6 sm:p-8 animate-in fade-in duration-200">
              ...
              <div className="rounded-2xl border border-steel/20 bg-paper p-6 sm:p-8 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {earnedBadges.map((b) => (
                    <div
                      key={b.id}
                      className="group rounded-xl border border-bridge-gold/30 bg-bridge-gold/10 p-4 transition-all duration-200 hover:border-bridge-gold/40"
                    >
                      <div className="flex items-center justify-between">
